...
public class g4_Controller : MonoBehaviour
{
    ...
    public List<GameObject> lineInWall;
    ...
    public List<GameObject> SlotChild;
    public GameObject RCTotal;
    private void Start()
    {
        Singleton = this;
        row = 7;
        ...
        {
            var rcIns = Instantiate(RCPrefab);
            RC.Add(rcIns);
            rcIns.transform.SetParent(RCTotal.transform);
        }
        for (int i = 0; i < SpawnMapController.Singleton.line.Count; i++)
        {
            Vector3 tmp = SpawnMapController.Singleton.line[i].transform.position;
            tmp.y += 0.77f;
            RC[i].transform.position = tmp;
        }
        ...
        for(int i = 0; i < slot.Count; i++)
        {
            ...
            {
                SlotChild.Add(slot[i].transform.GetChild(0).gameObject);
            }
        }
        SpawnLevel(4);
    }
    public void SpawnLevel(int level)
    {
                    ...
                    if (dem < 2)
                    {
                        ...
                        for (int j = 0; j < 3; j++)
                        {
                            ...
                            while (true)
                            {
                                ...
                                else if (d == 0 && cnt > 0 && MathF.Abs(rand_i - try_rand_ind[cnt - 1]) > 3)
                                {
                                    try_rand_ind.Add(rand_i);
                                    rand_index.Add(rand_i);
                                    break;
        ...
        {
            if (slot[i].transform.childCount > 0)
                SlotChild.Add(slot[i].transform.GetChild(0).transform.gameObject);
        }
        for (int i = 0; i < SlotChild.Count - 2; i++)
        {
            if (SlotChild[i].CompareTag(SlotChild[i + 1].tag) && SlotChild[i].CompareTag(SlotChild[i + 2].tag))
            {
                SceneManager.LoadScene("SampleScene");
            }
        }
    ...
    public void Win()
    {
        ...
        for(int i = 0; i < slot.Count; i++)
        {
            total += slot[i].transform.childCount;
        }
        if(total == 2)
        {
